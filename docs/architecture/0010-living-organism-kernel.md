# Living Organism Kernel (LOK) / Aethyro Organism Architecture

| Field | Value |
|-------|--------|
| **Document** | LOK Design — Living Organism Kernel |
| **Author** | (TBD) |
| **Date** | 2026-07-12 |
| **Status** | Draft (rev 3 — residual review: KD16 complete split) |
...
The Living Organism Kernel (LOK) is the unifying architecture that turns the existing Aethyro NTG research kernel — ternary compute, SIS graph, tamper-evident ledger, SovereignBrain multi-organ stack, multi-axis fitness — into a **software organism**: sparse event signaling, hierarchical nano-agency, immune detect/quarantine (with snapshot repair staged honestly), multi-stream awareness, and optional hardware **Body** adapters (CPU → MCU/neuromorphic → wetware).

LOK does **not** replace `SovereignBrain`, `Organ`, `SovereignFitnessContext`, or ADR 0002 rails. It composes them. Hot-path life signs use a **TelemetryRing** of binary events plus **HotCounters** (no JSONL on LD / score / select / activate). Cold-path export, dashboards, and audit remain text/JSONL — with selection JSONL still **record-primary** from `LoopStepRecord`, not reconstructed from 32-byte events alone.

**Selection authority is singular:** `SovereignFitnessContext` remains the only path that scores axes, accepts/rejects mutants, and writes the ledger for selection. **FitnessFederation** surfaces deterministic Immune `selection_veto` and ephemeral threshold bias only; L0 FitnessReports are dashboard/attention, not axis inputs. **Load-dependent** Immune signals (ring drops, heartbeat) may quarantine nano **proposals** but **never** block genomic `select_*`. **Assembly** owns organs and holds exactly one fitness context (one ledger writer for selection).

Every organic unit has a **NanoAgent** control handle (not a second owner of organ data). Nano-agents compose under **Assembly** into one complete-brain **OrganismView**. **EyeOrgan** and **ImmuneOrgan** are first-class `Organ` implementors. **AwarenessBus** makes “360° hyper-awareness” multi-stream observability with declared SLAs, not omniscience.

**Actionable path:** ship software-only organism loops on the current Rust stack first (L0–L1); treat neuromorphic boards and Cortical Labs CL1-class wetware as **Body adapters** (L2–L3 research horizon), not prerequisites.
...
| Sovereign multi-organ | `genomic/sovereign_brain.rs`, `language_organ.rs`, `organ.rs` | Working set, LTM, `activate` / `activate_from_text`; only these two implement `Organ` today |
...
ADR 0009 accepted SovereignBrain as multi-organ substrate. ADR 0008 proposed (ideas only) real-time self-awareness, self-healing via StateBlots, and long-horizon synthetic-biology/robotics — without implementation. Harden pass (SOVEREIGN_HARDEN.md) closed foundation gaps: Organ trait, shared loop, durable snapshot, JSONL.
...
2. **Organs are few and hand-wired.** Only `SovereignBrain` and `LanguageOrgan` implement `Organ`. No shared bus; no immune organ; no vision/sensor organ; agents (`ChromosomeAgent`, `DomainAgent`) are query handlers, not composable nano-brains.
3. **Self-heal is specified, not built.** ADR 0008 Part 3 describes StateSlot checkpoints + lineage rollback; code has `StateSlotStore::lineage` / `verify_lineage` but slots are not exposed on the ledger, and organ images are not in StateSlots. No ImmuneOrgan yet.
4. **“360° awareness” is a metaphor without a fabric.** Telemetry4D and StatsCollector exist; there is no multi-stream **AwarenessBus**, no ring buffer of binary events, no Eye fusion, no dashboard contract beyond HTML/JSONL sketches.
5. **Body is implicit (host CPU).** Neuromorphic sparsity and wetware embodiment are research context, not adapter traits.
6. **Composition risk.** Growing features without a hierarchical Assembly plan recreates a monolith — the opposite of “every piece has its own Agentic Intelligence.”

### 2.3 Why now

Sovereign stack is measured (train accept / prune reject under bio gates; ledger verifies; LTM activate hardened). The next leap is **architecture**, not another one-off organ. LOK freezes the organism model so implementation can proceed as ordered PRs without overclaiming product or biology.
...
| ID | Goal | Layer |
|----|------|-------|
| G1 | **Binary-first telemetry** on LD / score / select / activate hot paths via TelemetryRing + HotCounters; JSONL/text export off hot path only | L0 |
| G2 | **NanoAgent** control handles for every organic unit; hierarchical composition into **Assembly** (complete brain view), not a mega-module | L0 |
| G3 | **AwarenessBus** multi-stream observability + **EyeOrgan** sensor fusion for honest 360° hyper-awareness dashboards | L0 bus/eye; L1 dashboard |
| G4 | **ImmuneOrgan** detection + quarantine + ledger under ADR 0002; repair via **named snapshot checkpoints** (not StateSlots alone) | L0 detect/quarantine; L1 full restore |
| G5 | **Homeostasis** controller: multi-rate organism state views (structure, fitness, immune, awareness load) with sparse events | **L1 only** |
| G6 | **Body adapter** trait for CPU (now), MCU/neuromorphic (near), wetware CL1-class (horizon) without rewriting organs | L0 trait+HostCpu; L2/L3 stubs |
...
| NG2 | Magical omniscience cameras or unbounded surveillance; Eye is multi-sensor fusion of **declared streams** only |
...
| NG8 | Reconstructing full multi-axis `LoopStepRecord` from TelemetryEvent alone |
...
|-------|------|-----------|----------|
| **L0** | Invent-now software | Immediate PR series on current kernel | TelemetryRing + HotCounters, injection contract, AwarenessBus, NanoAgent handles, Assembly + FitnessFederation (weights/veto only), ImmuneOrgan detect/quarantine, EyeOrgan streams, HostCpuBody, OrganLive, ledger slot API + pre-select checkpoint helper, organism_demo |
| **L1** | Integrated organism loop | After L0 green tests + EXPERIMENTS.md | Snapshot restore repair, homeostasis controller, hyper-awareness HTML/SVG dashboard, eBPF optional reader (Linux only, feature-gated), organism_campaign overhead gates |
| **L2** | Hardware Body | Board available + adapter CI | Neuromorphic event bus adapter (spike/event I/O), MCU UART/SPI sparse telemetry, power/thermal sensors as Eye streams |
...
```mermaid
flowchart TB
  subgraph Body["Body adapters (L0 CPU → L2/L3)"]
    CPU[HostCpuBody]
    NEURO[NeuromorphicBody]
    WET[WetwareBody]
  end
  ...
  Eye[EyeOrgan]
  Bus[AwarenessBus]
  Ring[TelemetryRing + HotCounters]
  Imm[ImmuneOrgan]
  Asm[Assembly owns organs]
  ...
  NA3[NanoHandle_Eye]
  NA4[NanoHandle_Imm]
  Fed[FitnessFederation weights+veto only]
...
**Design:** Fixed-capacity ring of packed binary **events** plus sibling **HotCounters** (atomics). Producers: instrumented LD (sampled), score/select/activate (via context-held refs), NanoAgent heartbeats, Immune detectors. Consumers: AwarenessBus sampler, offline exporters, optional eBPF-side reader, dashboard tailer.

```text
// Conceptual layout — implement under kernel/src/genomic/organism/
#[repr(C)]
...
```rust
pub struct TelemetryRing { /* atomic head/tail, Box<[TelemetryEvent]> */ }

impl TelemetryRing {
    pub fn try_push(&self, e: TelemetryEvent) -> bool; // drop-on-full, never block hot path
    pub fn drain_to(&self, out: &mut Vec<TelemetryEvent>, max: usize) -> usize;
}
...
**No process-global `organism_ring()`.** Preferred wiring:

1. **`SovereignFitnessContext` holds optional telemetry handles:**
   ```rust
   // conceptual fields on SovereignFitnessContext
   pub telemetry: Option<TelemetryHandles>, // ring + counters, shared via Arc or &
   ```
2. **`score` / `select_*` / organism paths** push events when `telemetry` is `Some`; zero cost when `None` (production default for micro-tests).
3. **`SovereignBrain::activate`:** either (a) optional parameter `telem: Option<&TelemetryHandles>` on a thin wrapper `activate_traced`, keeping existing `activate` signature for callers, or (b) optional field on brain set by Assembly before tick. Prefer **(a) wrappers** to avoid changing every call site semantics.
4. **LD path:** do **not** plumb `&TelemetryRing` through every popcount call. L0 default:
   - **HotCounters only** on LD (atomic `ld_pairs` += N), sampled every 1:N batch completion.
   - Full `TelemetryEvent` for LD only if single-threaded campaign mode **or** after MPSC ring lands.
5. **Assembly** owns the ring + counters; passes `TelemetryHandles` into context and wrappers at tick start.
...
| Rayon-parallel LD | **HotCounters only** (Relaxed atomics); no multi-writer ring until MPSC |
| Multi-thread organism (future) | Per-thread rings + merge drain, **or** MPSC ring with CAS slot claim — L1+ |

L0 scheduling of NanoAgents is **deterministic round-robin on one thread** (see §4.3.2). Parallel ticks deferred.
...
|----------|------|-----------------|
| LD bitparallel path | `genomic/ld_compute.rs`, `bitsliced_genotypes.rs` | HotCounters + sampled 1:N; no per-pair event |
...
#### 4.3.2 NanoAgent (“nano brain” control handle)
...
**Ownership (KD14):** **Assembly owns organs.** NanoAgent is a **control handle**, not `NanoAgent<O: Organ> { organ: O }`.
...
pub trait NanoBrain {
    ...
    fn tick(&mut self, asm: &mut AssemblyView, bus: &mut AwarenessBus) -> NanoTickResult;
    fn local_fitness(&self, asm: &AssemblyView) -> MultiAxisFitness;
    fn propose(&self) -> Option<NanoProposal>; // inert until Assembly + SelfMod path
}
```

**StateSlot mapping:** `desires: i32` remains the ledger store’s arbitrary encoding field — **summary signal only**. Authoritative goals live in `LocalGoal` on the handle (or Assembly side table).

**`ChromosomeAgent` / `DomainAgent`:** remain **query adapters / nano precursors**. They must **not** become second owners of `ChromosomeBrain` maps already held in `SovereignBrain`. If wrapped, they borrow or re-query through Assembly’s brain reference.

**Scheduling (L0):** deterministic **round-robin** `tick` on a single thread. Sequence diagrams that say “parallel” are **L1+ deferred** only. Parallel `&mut` ticks on shared organs are not free and are out of L0 scope.

NanoAgents are **not** LLMs. They are bounded control automata with optional rule-based proposals — same epistemic shape as `MutationRule` / selection loop.

#### 4.3.3 Assembly (complete brain composition) & selection authority

**Problem:** Hierarchical composition into one complete brain without a monolith; avoid dual fitness ownership.
...
| Aggregate nano reports for dashboard / attention | **`FitnessFederation`** | L0: does **not** alter axis values; may surface `selection_veto` from **deterministic** Immune only; does **not** call `log_mutation` for selection |
| Detect/quarantine; log `immune_*` status | **`ImmuneOrgan`** | Split outcomes: **selection_veto** (deterministic only) vs **load quarantine** (observability / proposal freeze only) — see §4.3.6 / KD16 |
| Own organs + one context + ring/bus | **`Assembly`** | Single place that constructs `SovereignFitnessContext` / ledger |

**Call graph:**

```text
organism_tick(budget):
  ...
  fed = federation.combine(reports, immune.selection_veto())  # veto from det. Immune only
  ...
  else if fed.veto_accept:   # deterministic Immune only
    ledger immune v1 action=selection_blocked ... (no topology change)
  # load_quarantined nanos: proposals frozen; genomic select_* still runs
```

**`run_selection_loop` coexistence:** remains the **standalone** API for demos/campaigns that only touch brain+context. `run_organism_loop` wraps Assembly and **calls into the same** `select_*` methods on the Assembly-owned context — never a parallel scorer. Double-scoring is avoided by: one context, one select call per step, federation runs **before** select as hint only.

**L0 FitnessReport policy (KD8/KD15):** `FitnessReport` / `local_fitness` samples are **observability, dashboard, and Immune attention only**. They **do not** alter `MultiAxisFitness` axis values produced by `SovereignFitnessContext::score`. `per_organ` blend weights are **reserved L1; ignored by `federate()` in L0**.

**Eye/Immune and MultiAxisFitness (KD8 preserved):**
...
| Eye coverage shortfall | **dashboard / stress flag only in L0** — does **not** change `min_utility_delta` or skip select |
| Immune **selection_veto** (deterministic only) | may skip `select_*` or force-reject; see detector class table §4.3.6 |
| Immune **load quarantine** (ring/heartbeat) | freeze that nano’s **proposals** only; **never** blocks genomic `select_*` |

No parallel utility formula.

**Registry:**

1. Organs owned by Assembly: `SovereignBrain`, `LanguageOrgan`, `EyeOrgan`, `ImmuneOrgan`, …
2. NanoHandles keyed by `agent_id`
3. Shared `AwarenessBus` + `TelemetryRing` + `HotCounters`
4. Exactly one `SovereignFitnessContext` (ledger writer for selection)
5. Immune may append ledger entries via context helper `log_immune_event(...)` that uses the **same** ledger

```mermaid
sequenceDiagram
  participant Host
  participant Asm as Assembly
  participant NA as NanoHandles
  participant Bus as AwarenessBus
  participant Fed as FitnessFederation
  participant Ctx as SovereignFitnessContext
  participant Led as Ledger
  participant Imm as ImmuneOrgan

  Host->>Asm: organism_tick(budget)
...
| `NanoProposal` | Suggested change; inert until rails + context path; frozen if load-quarantined |
| `QuarantineOrder` | Issued only by Immune; class = `Load` or `Deterministic` |
| `BodyFrame` | Body adapter I/O envelope |

##### AssemblyView (L0 nano tick surface)

`NanoBrain::tick` takes `&mut AssemblyView` so handles never own organs. L0 methods (keep small to avoid borrow fights in round-robin):

```rust
/// Read-mostly façade over Assembly for one nano tick. Mutating methods only
/// touch bus/handle tables — not SovereignBrain topology (selection owns that).
pub struct AssemblyView<'a> { /* internal refs */ }
...
impl AssemblyView<'_> {
    ...
    pub fn push_signal(&mut self, sig: NeuroSignal); // into AwarenessBus
    pub fn counters(&self) -> &HotCounters;           // read-only rates for dashboards
    // L0: no activate_mut / no select / no ledger write from nanos
}
```

**Complete brain (end state):** Assembly presents **OrganismView** (`score` → context, `activate` → brain wrappers, `tick`, `snapshot`). One interface, multiple organs — not one source file.
...
**360° hyper-awareness (honest definition):**

> Continuous fusion of **all registered streams** into a dashboard and Immune attention set, with time as the animating dimension. Coverage is **declared stream completeness vs SLA**, not omniscience. “Nothing gets past awareness” means: no registered stream is unsampled beyond its SLA, and drops are themselves visible stress signals.

#### 4.3.5 EyeOrgan (vision / multi-sensor fusion)

**Not** magic cameras only. Eye is the sensory cortex of **declared streams**.
...
| eye_extra health (drop rates, FS) | 0.10 |
...
impl Organ for EyeOrgan { /* kind = "eye_awareness" */ }
...
#### 4.3.6 ImmuneOrgan (self-heal analog — honest repair model)
...
| Mechanism | What it stores | Can restore organs? |
|-----------|----------------|---------------------|
...
| Pre-select **checkpoint** (new) | Named dir from `save_snapshot` (+ assembly_meta) taken **before** risky select | **Yes for what snapshot covers**; document restore limits in EXPERIMENTS |
| Ledger chain | mutation descriptions, fingerprints, traces | Audit/verify only — not organ image |

**Staged immune capability:**

| Stage | Behavior |
|-------|----------|
| **L0** | Detect + **quarantine** with **class split** (below) + mandatory ledger `immune_*` on Live↔Quarantined. Synthetic/forced detectors for tests. **No claim of brain heal via StateBlots.** |
...
impl TamperEvidentLedger {
    ...
    pub fn log_immune_event(&mut self, grammar: &str, structure_fp: u64) -> Result<u64, NtgError>;
}

// PR-5b contract for log_immune_event:
// 1. MUST call existing log_mutation (same chain / SignedEntry / traces map).
// 2. description = immune grammar string (§8.3).
...
// 6. Unit test: N immune events then verify_full_ledger() == Ok.
```
...
##### L0 Immune implementation shape (cells as docs only)

No five empty white-cell types in L0. Single organ:
...
| Eye coverage shortfall | coverage &lt; 1 | **Observability** | **No** | No (stress flag only) | Dashboard |

**Hard rule (Immune + KD16):**  
`selection_veto` ⇔ at least one **Deterministic** detector fired.  
Observability detectors **must not** skip `select_train_step` / `select_prune_step` / `select_child`, must not force `should_accept == false` via veto, and must not quarantine the **genomic selection pipeline** (Assembly still calls context select). Load quarantine freezes **`NanoProposal` admission** and optional non-selection mutators for that agent only.

**Actions:**

1. **Load quarantine (L0):** `NanoStatus::Quarantined` + `QuarantineClass::Load`; block `NanoProposal` apply for that agent; **genomic `select_*` continues**.
...
#### 4.3.7 Multi-rate organism state views (ex–“digital twin”)

Implementer-facing name: **multi-rate organism state views**. Biology “digital twin” language is **doctrine only** (G5 is L1).

| Rate | Content | Update |
|------|---------|--------|
| Micro | HotCounters, sampled TelemetryEvents | every N ops / ms |
| Meso | organ fingerprints, nano fitness | every organism_tick |
| Macro | MultiAxisFitness utility, Immune state, generation | every selection step |
| Meta | durable snapshot + ledger head hash | on consolidate / checkpoint |

**Homeostasis controller (L1 PR-11):** adjusts working-set capacity / prune_frac / sample rates / Immune thresholds as **proposals** under rails. **Stress phenotype (L1):** reduced activate capacity + raised `min_utility_delta` + higher telemetry sample rate when Immune quarantine or Body stress flags fire. Measure via existing task/bio axes.

**Durable genome:** ternary DNA nodes + sovereign snapshot + assembly_meta. Mutants ledgered via context.
...
/// L0 HostCpuBody: portable no-op sensors.
...
pub struct HostCpuBody { pub best_effort_proc: bool }
...
**Consumes:** optional ring drain annotations, `Telemetry4D`, Immune state, Eye coverage, **record-primary** selection JSONL, ledger verify waveform.

| View | Data |
|------|------|
| Radar / spider | task, bio, safety, structure_good, immune_health, eye_coverage |
| Bars | HotCounters rates |
| Waves | utility over generations; ledger verify flatline |
| Matrix | organ × metric heatmap |
| Graph | Assembly nano topology |
...
kernel/src/
  genomic/
    organism/              # ALWAYS compiled (like sovereign stack) — no default feature gate
      mod.rs
      telemetry_ring.rs
      awareness_bus.rs
      nano_agent.rs
  ...
  ntg/
    ledger/                # public slot + immune log helpers (PR-5b)
    observability/
    mutation/              # SelfModConfig remains OFF default
  bin/
    organism_demo.rs       # [[bin]] always registered like sovereign_*
    organism_campaign.rs   # L1
```
...
|---------|---------|-------------------------------------|----------------------------------|
| **Observability** | ring timestamps, ring_drops, bus drop order, heartbeat wall-clock, `/proc`, Eye coverage, Load quarantine | **No** | **No** |
...
**Selection purity:** Given the same brain topology, same frozen references, same evaluator baselines, and same deterministic Immune inputs (ledger integrity + scored axes + test inject flag), `select_*` either runs or is vetoed **identically** across hosts/loads. Ring pressure and heartbeat silence may change dashboard stress and which nanos may **propose**, but two identical campaigns under different CPU load must not diverge on whether train/prune selection executes.

Immune may use wall time for **Load quarantine rate limiting** only; the ledgered outcome (class + reason) is what auditors read for status changes.

---

## 5. API / Interface Changes

### 5.1 OrganLive (backward compatible)

Keep existing `Organ` unchanged. Add:

```rust
pub trait OrganLive: Organ {
    fn on_bus_signal(&mut self, sig: &NeuroSignal) {}
    fn health(&self) -> f32 { 1.0 }
}
```

Landed in **PR-5c** with Eye/Immune; SovereignBrain/LanguageOrgan get adapters when needed.

### 5.2 Selection loop

- Keep `run_selection_loop` signature and **record-primary** JSONL.
- Optional telemetry annotation fields when context has handles.
- Add `run_organism_loop(assembly, steps, export)` that calls the same `select_*` APIs.
...
pub struct FederationWeights {
    ...
    /// L0: must be 0.0 unless sourced from deterministic config (not Eye/load).
    pub min_utility_delta_bias: f32,
}

pub struct FederationHint {
    /// True only from Immune Deterministic selection_veto — never from ring/heartbeat.
...
### 5.5 Ledger — `log_immune_event` is `log_mutation`
...
5. Test: append many immune events → `verify_full_ledger()` Ok.
...
| immune events | ledger via structured grammar | mandatory on status change |
| `assembly_meta.json` | organ list, agent_ids, body kind | |
| StateSlots | existing 48-byte; desires = summary i32 | nano control lineage after PR-5b |
| checkpoint dirs | `save_snapshot` + assembly_meta | pre-select; restore limits documented |

### 6.2 Migration

- No breaking change to `ltm.jsonl` / `calib.wire` / `meta.json`.
- Assembly load: restore sovereign snapshot then reattach Eye/Immune; chromosomes still need re-ingest for full topology.
...
|-----------|------------|
| TelemetryRing 64K × 32 B | 2 MiB fixed |
| AwarenessBus queues | < 1 MiB |
| Immune memory codes | < 256 KiB |
...
- **Decision:** Reject; Assembly registry + protocols.
...
### A6. Extend StatsCollector / HotCounters only (no event ring)

- **Pros:** Minimal code; already lock-free; enough for rate dashboards.
- **Cons:** No typed anomaly kinds, no begin/end spans, weak Immune forensics, hard to correlate accept/reject with generation without side tables.
- **Decision:** **HotCounters are mandatory; ring is L0 for typed events.** Counters-only is insufficient as the sole L0 plan but is the **correct LD parallel path** until MPSC.
...
### A8. Immune as selection_loop middleware only (not an Organ); dashboard from JSONL only

- **Pros:** Smaller L0; detectors next to existing select; dashboard can ship from current JSONL without bus.
- **Cons:** Breaks composition symmetry (Eye/Immune as organs); harder multi-organ quarantine; middleware tends to fork selection_loop.
- **Decision:** **Immune is an Organ** for Assembly symmetry, but **first detectors may be invoked from the organism/selection path** (patrol called around select). Dashboard MVP may use JSONL alone (L1 can start thin); full radar with eye_coverage needs Eye + export.
...
|-------|----------------|----------------------|----------------------------|------------------|---------------------|
| Host binary / operator | via Assembly API | via Assembly API | constructs context; explicit | yes | only by config flag |
| Assembly root | receives Immune orders | yes (operator policy) | calls context.select_* | via context | respects SelfModConfig |
| ImmuneOrgan | **yes** (only path for auto quarantine) | **no** (Assembly/Host only) | **no** | immune events only | **no** |
| NanoHandle | **no** | **no** | propose only (inert) | **no** | **no** |
| EyeOrgan | **no** | **no** | **no** | **no** | **no** |
...
| Immune false positive storm | Medium | `immune_slack`, `max_quarantines_per_sec` (default 8), severity thresholds |
| Immune / assembly host code rewrite | Critical | **Forbidden** |
| eBPF privilege | High | Feature-gated, off default, no remote export |
| Eye media privacy | High | Explicit opt-in streams; local only |
| Ledger spoof | High | SHA-256 chain verify; critical Immune detector |
| NanoProposal flood | Medium | Admission budget: max proposals per tick (default 2); SelfMod still off |
| Dual ledger writers | Medium | Single context-owned ledger; Immune uses same handle |

### 8.3 Structured immune ledger grammar (mandatory)
...
```text
immune v1 action=quarantine class=deterministic agent=0xAE000003 reason=ledger_verify code=1 gen=12 prev=live
immune v1 action=quarantine class=load agent=0xAE000007 reason=ring_drops code=3 gen=12 prev=live
immune v1 action=selection_blocked class=deterministic reason=ledger_verify code=1 gen=12
immune v1 action=release agent=0xAE000003 reason=operator gen=13 prev=quarantined
immune v1 action=checkpoint_restore path=... verify=OK gen=14
immune v1 action=detect_only class=load reason=ring_drops code=3
```
...
| Signal | Source | Alert / Immune |
|--------|--------|----------------|
| ops/s, path mix | StatsCollector | structural stream |
| ring_drops | HotCounters | medium anomaly |
| utility waveform | selection records | evolutionary stream |
| ledger verify | TamperEvidentLedger | critical if fail |
| eye coverage | EyeOrgan | medium if < 1.0 sustained |
| quarantine count | ImmuneOrgan | high if rising |
...
| PR-5 | NanoHandle does not own organs; desires packing; AssemblyView method surface compiles |
| PR-5b | Ledger slot write/lineage/verify; `log_immune_event` → verify_full_ledger after N events; no private-field hacks |
| PR-5c | OrganLive default methods; Eye/Immune kind strings |
...
| PR-9 | organism_demo smoke; HostCpuBody health constant-OK |
| PR-10+ | dashboard snapshot smoke; campaign overhead; checkpoint restore integration (L1) |

**Immune drill (L0):** pure unit/integration test with forced **Deterministic** detector — not only a demo bin. Separate test: Observability detector never sets `selection_veto`.
...
2. **Staged:** ring+bench → measure LD → instrument → bus → nanos → ledger slots → assembly → eye/immune → demo → dashboard → campaign → eBPF spike → eBPF feature.
3. **Self-mod:** remains OFF.
4. **Rollback:** stop constructing Assembly telemetry handles; sovereign stack unchanged. Quarantine reversible by Host/Assembly. Ring ephemeral.
...
3. **Clear-quarantine policy:** auto-release after N healthy ticks vs operator-only — default **operator/Assembly explicit release** for L0 safety; auto-release can be L1 config.
...
| `kernel/src/genomic/organ.rs` | Organ trait (two impls today) |
...
| # | Decision | Rationale |
|---|----------|-----------|
| KD1 | **LOK is composition, not rewrite** of SovereignBrain / ledger / multi_axis | Preserve measured stack |
| KD2 | **TelemetryRing + HotCounters on hot path; JSONL cold and record-primary** | Perf + EXPERIMENTS parity |
| KD3 | **NanoHandle per organ key; Assembly owns organs** | No double-own; hierarchical complete brain |
| KD4 | **360° awareness = multi-stream coverage vs SLA + fusion, not omniscience** | Honest engineering |
| KD5 | **Immune owns detect/quarantine + ledger; repair is named snapshot-checkpoint restore under rails — StateSlots are control lineage only, not organ images** | Matches real APIs and persist limits |
| KD6 | **Self-mod remains OFF by default; Immune cannot bypass rails or clear its own quarantine** | ADR 0002 + privilege matrix |
| KD7 | **Body adapters stage hardware; L0 HostCpuBody constant-OK health** | Ship software organism first |
...
| KD14 | **L0 nano scheduling = single-thread deterministic round-robin** | Avoid parallel `&mut` hazards |
| KD15 | **Single selection authority = SovereignFitnessContext; Assembly holds one instance; federation feeds hints only** | No dual fitness ownership |
| KD16 | **Complete split:** observability (ring/bus/heartbeat/load quarantine) never blocks or force-rejects `select_*`; only **Deterministic** Immune detectors set `selection_veto`. Ring/bus timestamps/drops never enter fingerprints or axes | ADR 0002 rail 4 under load |
| KD17 | **Module path = `genomic/organism/`; per-Assembly ring (not process-global); L0 modules always compiled** | Freeze former open Q1–Q3 |
| KD18 | **L0 Immune = detector list + config + quarantine; white-cell names are documentation only** | Avoid empty type sprawl |
...
| KD21 | **`log_immune_event` = `log_mutation` wrapper** with minimal ExecutionTrace; no second audit channel | Ledger integrity |
...
|----|-------|----------|---------|-------|
| **PR-1** | TelemetryRing + HotCounters | Fixed ring, schema_version in flags, unit tests, **try_push microbench** published | — | L0 |
...
| **PR-4** | AwarenessBus + NeuroSignal | Bus, drop counters, single-thread order tests | PR-1 | L0 |
| **PR-5** | NanoHandle + LocalGoal + adapters | No organ ownership in handle; ChromosomeAgent stays query adapter | PR-4 | L0 |
| **PR-5b** | Ledger public slot API + pre-select checkpoint helper | `write_agent_slot`, lineage, `log_immune_event`; `checkpoint_before_select(dir)` using `save_snapshot` | — (parallel after PR-1 ok) | L0 |
| **PR-5c** | OrganLive trait | Default methods; used by Eye/Immune | PR-5 | L0 |
| **PR-6** | Assembly + FitnessFederation | Registry, federate→hint, organism_tick call graph, single context | PR-5, PR-5b | L0 |
| **PR-7** | EyeOrgan v0 | Streams + SLA defaults + coverage + fusion weights | PR-4, PR-6 | L0 |
| **PR-8** | ImmuneOrgan v0 | Detectors, quarantine, rate limits, ledger grammar; **forced-detector unit test**; repair **not** required | PR-6, PR-5b | L0 |
| **PR-9** | HostCpuBody + Body trait + organism_demo | Constant-OK health; end-to-end tick; assembly_meta; optional neuromorphic/wetware **stubs** in same PR if zero-cost | PR-6–8 | L0 |
| **PR-10** | Hyper-awareness HTML dashboard | Radar/bars/waves; JSONL and/or exports | PR-3, PR-7 | L1 |
...
| **PR-12** | organism_campaign + EXPERIMENTS | Multi-chr + immune stress + overhead | PR-9 | L1 |
...
| **PR-14** | ADR 0010 LOK + STATUS sync | After measurements | PR-12 | docs |

**L0 done definition:** PR-1…PR-9 merged; `cargo test` green; organism_demo shows Live nanos + Eye coverage export; **Immune detect + quarantine + ledger `immune_*` drill** (forced detector unit test); self-mod still default off; **full snapshot repair is L1 (PR-11b), not L0 gate.**
...
pub fn activate_traced<'a>(
    ...
    telem: Option<&TelemetryHandles>,
) -> &'a WorkingSet {
    let ws = brain.activate(query, chr_filter);
    if let Some(h) = telem {
        let _ = h.ring.try_push(TelemetryEvent {
...
### 17.2 Immune ledger description
...
if hint.veto_accept { // immune.selection_veto only (Deterministic)
    ctx.ledger.log_immune_event(
        "immune v1 action=selection_blocked class=deterministic ...",
...
## 18. Mapping: user pillars → LOK

| User pillar | LOK answer |
|-------------|------------|
| TelemetryRing + hot counters | §4.3.1; KD13; PR-1–2 |
| Neuromorphic + wetware + existing stack | Body adapters §4.4; L0 HostCpu |
| Living running organism | Multi-rate views §4.3.7; metaphor disciplined; homeostasis L1 |
| Agentic nano-agents → complete brain | NanoHandle + Assembly §4.3.2–4.3.3; KD14–15 |
| Advanced RT visualization | Dashboard §4.5; PR-10 |
| EYE 360° awareness | EyeOrgan + SLA/coverage §4.3.4–4.3.5 |
| Biological self-heal immune | ImmuneOrgan §4.3.6 staged repair model |
...
- **Language guard:** “organism architecture / multi-rate control,” not “sentient” or “alive.”
...
- **Rev 3** closes residuals: KD16 complete detector-class split (load ≠ selection_veto); FitnessReport L0 observability-only; ephemeral federation bias; `log_immune_event` = `log_mutation`; AssemblyView L0 methods; PR-2 pin to `ld_simd_bench`.
