...
#[cfg(test)]
mod tests {
    use ntg::ntg::operators::genomic::{GenomicOperator, GenomicNode};
    use ntg::ntg::operators::genomic_loader::{GenomicBatch, GenomicFormat, GenomicPipeline};
    ...
    fn test_set_get_genotypes() {
        ...
        // Test setting individual genotypes
    ...
    fn test_statistics_computation() {
        ...
        // Fill with realistic genotype data
        for snp in 0..20 {
            for ind in 0..100 {
        ...
        // All means should be in valid range [0, 2]
        for mean in &op.means {
            assert!(*mean >= 0.0 && *mean <= 2.0);
        }

        // All std_devs should be positive
    ...
    fn test_prs_computation() {
        ...
        // All PRS should be non-negative
        for score in &prs {
            assert!(*score >= 0.0);
        }
    }
    ...
    fn test_missing_data_rate() {
        ...
        // Set some missing data (value 3)
        for i in 0..10 {
            op.set(0, i, 3); // 10% missing in first SNP
        }

        let missing_rate = op.estimate_missing_rate();
        let expected_rate = 10.0 / (100.0 * 10.0); // 10 missing out of 1000 total
    ...
    fn test_allele_frequency_calculation() {
        ...
        // Create SNP with known allele frequency: 30% alternate
        // This means genotypes should average to 0.6
        for ind in 0..70 {
        ...
        let mean = op.means[0];
        let expected_mean = (0.0 * 70.0 + 2.0 * 30.0) / 100.0; // = 0.6
        assert!((mean - expected_mean).abs() < 0.01);

        let allele_freq = op.allele_freqs[0];
        let expected_af = expected_mean / 2.0; // = 0.3
        assert!((allele_freq - expected_af).abs() < 0.01);
    }

    #[test]
    fn test_bulk_load_export() {
        let mut op = GenomicOperator::new(20, 10);

        // Create bulk genotype array
        let mut genotypes = vec![0u8; 20 * 10];
        for i in 0..genotypes.len() {
            genotypes[i] = (i % 3) as u8;
        }

        // Test that bulk operations work (basic smoke test)
        // In real FFI usage, these would be called via C interface
